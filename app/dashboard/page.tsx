...
interface QuickAction {
  ...
  label: string;
  subLabel: string;
  // FIX 1: Change type to allow Component references
  icon: any; 
  gradient: string;
  path: string;
  keywords: string[];
  ...
  const quickActions: QuickAction[] = [
    {
      ...
      // FIX 2: Pass the component name directly (No < > wrappers)
      icon: Plus, 
      ...
      <div className="relative z-10 px-4 sm:px-8 lg:px-12 xl:px-20 pb-12 sm:pb-24 max-w-[1920px] mx-auto">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-center px-4 py-12 sm:px-6 sm:py-20 space-y-8 sm:space-y-10">
            <div className="space-y-4 max-w-3xl">
              ...
              >
                 <span>A Creative Operations Apllication</span>
              </motion.div>
            ...
            <div className="w-full max-w-2xl relative group/search">
              ...
              <div className="relative flex items-center bg-white/10 backdrop-blur-xl border border-white/20 rounded-full p-1.5 shadow-lg transition-all focus-within:bg-white/15 focus-within:border-white/40 focus-within:scale-[1.01]">
                ...
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery("")} className="p-2 text-white/70 hover:text-white">
                    <X size={16} />
                  </button>
                )}
                <button className="h-9 sm:h-11 px-4 sm:px-6 rounded-full bg-white text-indigo-600 font-semibold text-xs sm:text-sm hover:bg-blue-50 transition-colors shadow-sm">
                  Search
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3 w-full">
               {heroFilters.map((filter, idx) => (
                 <motion.button 
                   ...
                   className="flex items-center gap-1.5 px-3 sm:px-5 py-2 sm:py-2.5 rounded-full bg-black/20 hover:bg-black/30 border border-white/10 text-white text-xs sm:text-sm font-medium transition-all backdrop-blur-sm"
                 >
                  ...
                  >
                    ...
                    <div className="relative h-full p-4 sm:p-6 flex flex-col justify-between z-10">
                      <div className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg bg-gradient-to-br ${action.gradient} transform transition-transform duration-500 group-hover:rotate-6 group-hover:scale-110`}>
                          ...
                          {/* FIX 3: Render Icon Directly with Tailwind Classes */}
        ...
        <section className="w-full relative">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 px-1 gap-4">
             ...
             <button className="group w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bg-white border border-slate-200 text-slate-600 text-sm font-semibold hover:border-indigo-200 hover:text-indigo-600 hover:shadow-md transition-all">
