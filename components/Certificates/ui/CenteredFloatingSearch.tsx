...
import { Bell, Home, User, Search, X } from 'lucide-react'
...
  // Existing state for hover/expand
...
    if (e.key === 'Enter') {
      // Optional: Implement actual search/submit logic here
      console.log('Searching for:', searchQuery)
      inputRef.current?.blur() // Optional: Lose focus after search
    }
  }

  // Manage expand/collapse state with delay (Existing Logic)
  useEffect(() => {
...
          )}
        >
          {/* Search Section: Updated to include input state and clear button */}
...
                className="flex-grow w-full bg-transparent text-white placeholder:text-white/60 text-sm pl-2 pr-4 py-2 outline-none border-none"
                aria-label="Search input"
              />
              
              {/* Clear Button (Using Lucide X for design consistency) */}
              {isExpanded && searchQuery.length > 0 && (
                <button
                  onClick={handleClear}
                  aria-label="Clear Search"
                  className="ml-1 mr-2 p-1 rounded-full text-white/60 hover:text-white hover:bg-white/10 transition-colors duration-200 flex-shrink-0"
                >
                  <X size={16} />
                </button>
              )}
...
  {/* Home Icon - FIXED: Replaced Link with <a> */}
...
  {/* User Account Icon - FIXED: Replaced Link with <a> */}
...
  <Search size={20} />
  
  {/* Home with link - FIXED: Replaced Link with <a> */}
  <a href="/dashboard">
